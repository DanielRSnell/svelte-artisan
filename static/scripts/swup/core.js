!(function (t, e) {
	...
	const p = function (t) {
			...
			if (n)
				return (
					this.triggerEvent('transitionEnd', e), this.cleanupAnimationClasses(), [Promise.resolve()]
				);
			var i;
			(i = () => {
	...
	function E(t) {
		...
		return i.length
			? i
			: (console.warn(`[swup] No CSS animation duration defined on elements matching \`${e}\``),
			  [Promise.resolve()]);
	}
	function P(t, e) {
	...
	const S = function (t) {
		const e = ((t, e) => {
			...
			return (
				(n.innerHTML = ''), (n = null), { title: s, pageClass: a, blocks: i, originalContent: t }
			);
	...
	function U(n) {
		...
		(this.currentPageUrl = t()),
			Promise.all([h, ...u])
				...
				.catch((t) => {
					void 0 !== t &&
						((this.options.skipPopStateHandling = () => ((window.location = t), !0)),
						history.go(-1));
				});
	}
	const L = function (t) {
	...
	const _ = function (n, i) {
		...
		if (
			(document.documentElement.classList.remove('is-leaving'), !this.isSameResolvedUrl(t(), n.url))
		)
			return;
		const { url: r } = c.fromUrl(n.responseURL);
		this.isSameResolvedUrl(t(), r) ||
			(this.cache.cacheUrl({ ...n, url: r }), (this.currentPageUrl = t()), e(r)),
	...
	return class {
		...
		enable() {
			'undefined' != typeof Promise
				? ((this.delegatedListeners.click = s(
		...
		linkClickHandler(e) {
			...
			if (e.metaKey || e.ctrlKey || e.shiftKey || e.altKey)
				return void this.triggerEvent('openPageInNewTab', e);
		...
		handleLinkToSamePage(t, n, i) {
			if (n) {
				if ((this.triggerEvent('samePageWithHash', i), !g(n)))
					return console.warn(`Element for offset not found (#${n})`);
				e(t + n);
		...
		popStateHandler(e) {
			...
			s ? (this.scrollToElement = s) : e.preventDefault(),
				...
				this.options.animateHistoryBrowsing ||
					(document.documentElement.classList.remove('is-animating'), h()),
